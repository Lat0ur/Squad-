...
HRESULT __fastcall hkPresentScene(IDXGISwapChain* pSwapChain, unsigned int SyncInterval, unsigned int Flags)
{
    ...
    if (Globals::Open)
    {
      ...
      //  ImGui::SetWindowSize(ImVec2(230, 300));
        ImGui::Begin(_("squad"));
        ImGui::Checkbox(_("draw health"), &esp);
        ImGui::Checkbox(_("draw bone"), &draw_bone);
        ImGui::Checkbox(_("draw_teammates"), &draw_teammates);
        ImGui::End();
    }
    else
    {
...
bool __stdcall DllMain(HMODULE dll_module, DWORD conclusion, LPVOID reserved)
{
    ...
    if (conclusion == 1)
    {
        ...
        init();
        Globals::Base = reinterpret_cast<uintptr_t>(GetModuleHandle(NULL));
    }
